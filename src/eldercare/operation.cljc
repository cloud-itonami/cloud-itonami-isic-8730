(ns eldercare.operation
  "OperationActor -- one residential-eldercare operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (EldercareOps-LLM) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Eldercare Governor
  ...
  One graph run = one residential-eldercare operation (intake ->
  ...
  decision to a human operator (the facility administrator/care manager).
  The approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:care-plan/finalize`/`:incident-response/finalize`
  ALWAYS reach this node when the governor is clean -- see `eldercare.
  phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [eldercare.eldercareopsllm :as eldercareopsllm]
            [eldercare.governor :as governor]
...
(defn build
  ...
  `eldercare.store/Store`).
  opts:
    :advisor      -- an `eldercare.eldercareopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (eldercareopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; EldercareOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (eldercareopsllm/-advise advisor store request)]
            {:proposal p :audit [(eldercareopsllm/trace request p)]})))

      ;; Eldercare Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
